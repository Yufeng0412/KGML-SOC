...
def changePadValue(y, seq_length):
    ...
    t = pack_padded_sequence(y, seq_length,batch_first=True, enforce_sorted=False)
...
# path_hid = '%s/model/gru-epoch50-batch256-231121_state_dict.pth'%(hidmodel_path)
...
model = netSOC.SOCGRU(n_x, n_hidden, n_layer, dropout=0.2)
model.to(device)
...
model_version='GRU_epoch{:d}_batch{:d}_{}'.format(n_epoch, batch_size, now)
